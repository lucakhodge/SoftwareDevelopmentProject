...
app.get("/searchResults", (req, res) => {
	...
	// if no input was given, return the whole database
	if (
		req.query.fileName == "" &&
		req.query.subject == "select" &&
		req.query.lectureNotes == null &&
		req.query.articles == null &&
		req.query.practiceTests == null
	) {
		find = "SELECT * FROM resultsCombined";
	}
	...
	db.any(q, [req.query.id])
		.then((data) => {
			...
			db.any(q2, [req.query.id, username])
				.then((data2) => {
					q3 = "SELECT COUNT(*) FROM LikedStudyGuides_to_Users WHERE SG_id = $1;";
					db.any(q3, [req.query.id])
						.then((data3) => {
                            liked = false;
                            // check if the user has liked the document being displayed
                            if (data2[0].count > 0) {
                                liked = true;
                            }
                            
                            // set like count in StudyGuides table
                            likes = data3[0].count;
							q4 = `UPDATE StudyGuides SET likes = ${likes} WHERE SG_id = $1;`;
							db.any(q4, [req.query.id]);
...
app.post("/like", (req, res) => {
	...
	q = "INSERT INTO LikedStudyGuides_to_Users (SG_id, username) values ($1, $2) ;";
...
app.post("/unlike", (req, res) => {
	...
	q = "DELETE FROM LikedStudyGuides_to_Users WHERE SG_id = $1 AND username = $2;";
	db.any(q, [req.body.id, username])
		.then((data) => {
			res.redirect("/display?id=" + req.body.id);
		})
	...
	//     "https://media-cldnry.s-nbcnews.com/image/upload/t_nbcnews-fp-1024-512,f_auto,q_auto:best/streams/2013/January/130122/1B5672956-g-hlt-130122-puppy-1143a.jpg";
		...
		db.any("SELECT * FROM StudyGuides")
			.then((result) => {
				...
				mes = req.body.fileName + " by " + username;
				res.render("pages/upload", { message: "Uploaded Sucessfully! " + mes });
			})
			.catch((error) => {
				console.error(error);
		...
		db.any("SELECT * FROM StudyGuides_to_Subjects")
			.then((result) => {
				console.log("Data from StudyGuides_to_Subjects:", result);
			})
			.catch((error) => {
				console.error(error);
